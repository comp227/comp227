...
Know that GitHub Copilot is now [available as a plugin for Webstorm](https://plugins.jetbrains.com/plugin/17718-github-copilot--your-ai-pair-programmer)
...
Once you install the plugin, you'll need to activate it via the signin process on the right.
![copilot welcom login window](../../images/1/custom/copilot_login.png)

Once you start the signin process, you'll need to transfer some additional tokens to github so that Webstorm and copilot can talk to each other:
![copilot signin codes](../../images/1/custom/copilot_signin.png)

Follow the directions that are posted there and once you finish logging in, you should be able to start a chat from Webstorm.
You can also start an inline chat from any line of code.
This can be done by right-clicking on a line and selecting ***GitHub Copilot->Copilot: Open Inline Chat***
![copilot right click menu for starting a chat](../../images/1/custom/copilot_enable_chat_here.png)

Copilot and other LLMs can be useful in a wide variety of scenarios.
For example, Copilot can be asked to generate code for an open file by describing the desired functionality in text:

![copilot input on vscode](../../images/1/gpt1.png)

After pressing enter, Copilot provides a suggested change for you to review:

![code added by copilot](../../images/1/gpt2.png)

In the case of our example, at first it looks good.
However, if we keep the suggestion at some point our IDE and it's helpful suggestions will start to flag some issues

![incorrect code generated by copilot](../../images/1/custom/copilot_suggestion_wrong_line.png)

Copilot is suggesting some additional handlers but is placing them in the incorrect spot.

We could copy only the parts that we need by selecting and right-clicking on the suggested code and selecting ***Copy Code Block***.
![copy code block for copilot with selected code](../../images/1/custom/copilot_copy_code_block.png)
Then closing the inline chat and pasting the correct code, *making sure we replace the older `return` statement*.

An event handler may also be generated.
By starting a comment with `//`, and then explaining what you want,
copilot will often provide you with suggestions for both what you want to build and then the code to build it.
You can see that here, where I just typed `//` said `finish` and then hit tab to see the suggested output.
...
Copilot is also useful for debugging.
Imagine we have the following error:

![error message for missing parenthesis from vite](../../images/1/custom/copilot_syntax_error.png)

If you copy that error message into Copilot's chat, not only will you get an explanation of the problem, *you'll also get a suggested fix*:
...
This may even lead into it spending some time trying to figure out and fix issues.
![copilot creating a login component progress part 1](../../images/1/custom/copilot_gpt6_a.png)

As it continues going through and editing your files
![copilot creating a login component progress part 2](../../images/1/custom/copilot_gpt6_b.png)

And even trying to install or elevate privileges to install necessary libraries until you have a result that works.
![copilot creating a login component progress part 3](../../images/1/custom/copilot_gpt6_c.png)
...
One major limitation of language models is that they are unable to implement changes across several files.
Language models are also currently unable to generalize code.
...
(even with minor adjustments), *the language model may fail to use them, or introduce subtle bugs*.
This deteriorates the code base quality because the language models often:

- duplicate functions
- duplicate components
- overcomplicate functions
- fail to reduce or refactor existing functions
- overengineer new components

Remember that most of this is because LLMs on the whole generate output, moreso than restructure and refactor existing code.
It is more common for an LLM to just add more code to fix a problem than it is to go back and fix what is truly ailing about it,
which *can result in a bigger mess and result in more technical debt with the code*.
